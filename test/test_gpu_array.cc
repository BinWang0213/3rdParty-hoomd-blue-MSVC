...
TEST_CASE("GPUArray_transfer_tests")
{
	std::shared_ptr<ExecutionConfiguration> exec_conf(new ExecutionConfiguration(ExecutionConfiguration::GPU));
...
TEST_CASE("GPUArray_resize_tests")
{
	std::shared_ptr<ExecutionConfiguration> exec_conf(new ExecutionConfiguration(ExecutionConfiguration::GPU));
...

	{
		...
		for (unsigned int i = 3; i < 17; i++)
			REQUIRE(h_handle.data[i] == (unsigned int)0);
		...
		for (unsigned int i = 3; i < 17; i++)
			REQUIRE(h_handle.data[i + pitch] == (unsigned int)0);

		// check that the two new rows are zero
		for (unsigned int i = 0; i < 17; i++)
		{
			REQUIRE(h_handle.data[i + 2 * pitch] == (unsigned int)0);
			REQUIRE(h_handle.data[i + 3 * pitch] == (unsigned int)0);
		}
